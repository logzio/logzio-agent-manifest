...
function add_logs_receivers_to_otel_config () {
    ...
    yq eval-all -ie 'select(fileIndex==0).receivers = select(fileIndex==1) | select(fileIndex==0)' $otel_config $logzio_temp_dir/logs_otel_receivers.yaml 2>$task_error_file
    if [[ $? -ne 0 ]]; then
        local err=$(cat $task_error_file)
    ...
    local receivers=$(yq e -e 'keys' $logzio_temp_dir/logs_otel_receivers.yaml 2>$task_error_file)
    local err=$(cat $task_error_file)
    if [[ ! -z "$err" ]]; then
    ...
    for receiver in $receivers; do
        ...
        yq e -ie ".service.pipelines.logs.receivers += \"$receiver\"" $otel_config 2>$task_error_file
        if [[ $? -ne 0 ]]; then
            local err=$(cat $task_error_file)
...
function add_logs_exporter_to_otel_config () {
    ...
    yq e -ie "logzio/logs.account_token = \"$logs_token\"" $logzio_temp_dir/logs_otel_exporter.yaml 2>$task_error_file
    if [[ $? -ne 0 ]]; then
        local err=$(cat $task_error_file)
    ...
    yq e -ie "logzio/logs.region = \"$logzio_region\"" $logzio_temp_dir/logs_otel_exporter.yaml 2>$task_error_file
    if [[ $? -ne 0 ]]; then
        local err=$(cat $task_error_file)
    ...
    yq e -ie '.service.pipelines.logs.exporters += "logzio/logs"' $otel_config 2>$task_error_file
    if [[ $? -ne 0 ]]; then
        local err=$(cat $task_error_file)

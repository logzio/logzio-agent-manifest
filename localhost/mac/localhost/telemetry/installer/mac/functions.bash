...
function run_otelcol_contrib_as_a_service () {
    ...
    cat $logzio_temp_dir/com.logzio.OTELCollector.plist > /Library/LaunchDaemons/com.logzio.OTELCollector.plist

    launchctl load /Library/LaunchDaemons/com.logzio.OTELCollector.plist >/dev/null 2>$task_error_file
    local err=$(cat $task_error_file)
    if [[ ! -z "$err" ]]; then

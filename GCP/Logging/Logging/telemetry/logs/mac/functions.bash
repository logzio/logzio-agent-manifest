...
function run_logzio_pubsub_integration(){
    ...
    cd $logzio_temp_dir/logzio-google-pubsub/ && ./run.sh --listener_url=$listener_url --token=$shipping_token --gcp_region=$region --log_type=gcp_agent --function_name="f$function_name" --resource_list=$resource_type 2>$task_error_file
    if [[ $? -ne 0 ]]; then
        local err=$(cat $task_error_file)

...
#   ---
# Output:
#   If PowerShell was not run as Administrator, writes error message and exits. Otherwise, does nothing. 
function Test-IsElevated {
    $local:ExitCode = 1

    $local:Message = 'Checking if PowerShell was run as Administrator ...'
    Send-LogToLogzio @{level = 'INFO'; message = $Message}
    Write-Log '[INFO]' $Message >> $LogFile
    ...
    Send-LogToLogzio @{level = 'ERROR'; message = $Message}
    Write-Error $Message

...
function get_agent_functions_scripts () {
    ...
    curl -fsSL $repo_url/scripts/linux/functions.bash > logzio-temp/agent_functions.bash 2>logzio-temp/task_result
    if [ $? -ne 0 ]; then
    ...
    curl -fsSL $repo_url/scripts/linux/utils_functions.bash > logzio-temp/utils_functions.bash 2>logzio-temp/task_result
    if [ $? -ne 0 ]; then
        cat logzio-temp/task_result >> logzio_agent.log

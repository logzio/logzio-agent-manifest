...
function get_arguments () {
    echo -e "[INFO] Getting arguments ..." >> logzio_agent.log
    ...
    while true; do
        case "$1" in
            ...
            --url=*)
                ...
                echo -e "[INFO] url = $app_url" >> logzio_agent.log
                ;;
            --id=*)
                ...
                echo -e "[INFO] id = $agent_id" >> logzio_agent.log
                ;;
            --debug=*)
                ...
                echo -e "[INFO] debug = $app_json_file" >> logzio_agent.log
                break
                ;;
            "")
                break
...
function check_validation () {
    echo -e "[INFO] Checking validation ..." >> logzio_agent.log
...
function install_update_homebrew () {
    echo -e "[INFO] Checking if Homebrew is installed ..." >> logzio_agent.log
    which brew >/dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo -e "[INFO] Installing Hombrew ..." >> logzio_agent.log
    ...
    echo -e "[INFO] Updating Hombrew ..." >> logzio_agent.log
...
#   Exit Code 3
function install_jq () {
    echo -e "[INFO] Checking if jq is installed ..." >> logzio_agent.log
    which jq >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        return
    fi

    echo -e "[INFO] Installing jq ..." >> logzio_agent.log
...
function get_app_json () {
    echo -e "[INFO] Getting application JSON ..." >> logzio_agent.log

    if [ ! -z "$app_json_file" ]; then
        # Using local app JSON file
        echo -e "[INFO] Using local application JSON file ..." >> logzio_agent.log
        cp $app_json_file logzio-temp/app.json
        return
    fi

    # Getting app JSON from agent
    echo -e "[INFO] Getting application JSON from agent ..." >> logzio_agent.log
...
function build_repo_path () {
    echo -e "[INFO] Building repo path ..." >> logzio_agent.log
    ...
    echo -e "[INFO] repo_path = $repo_path" >> logzio_agent.log
...
function get_prerequisite_scripts () {
    echo -e "[INFO] Getting prerequisites script file from logzio-agent-scripts repo ..." >> logzio_agent.log
    ...
    echo -e "[INFO] Getting prerequisites functions script file from logzio-agent-scripts repo ..." >> logzio_agent.log
...
function get_installer_scripts () {
    echo -e "[INFO] Getting installer script file from logzio-agent-scripts repo ..." >> logzio_agent.log
    ...
    echo -e "[INFO] Getting installer functions script file from logzio-agent-scripts repo ..." >> logzio_agent.log

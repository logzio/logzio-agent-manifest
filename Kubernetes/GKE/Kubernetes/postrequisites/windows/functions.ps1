...
function Test-AreAllPodsRunningOrCompleted {
    ...
    while ($retries -lt 3) {
        $retries++
        $local:podStatuses = kubectl -n monitoring get pods --no-headers -o custom-columns=":.status.phase" 2>$null
...
function Test-IsAnyPodPending {
    ...
    $local:err = ""
    $local:pods = kubectl -n monitoring get pods --no-headers -o custom-columns=":.metadata.name,:.status.phase" 2>$null | ForEach-Object {$_ -replace '\s+', ' '}
    foreach ($pod in $pods) {
        ...
        $local:eventReason = kubectl get event -n monitoring --field-selector involvedObject.name=$podName --no-headers -o custom-columns=":.reason" 2>$null | Select-Object -last 1 
        $local:eventMessage = kubectl get event -n monitoring --field-selector involvedObject.name=$podName --no-headers -o custom-columns=":.message" 2>$null | Select-Object -last 1 

...
function build_enable_metrics_helm_set () {
    echo -e "[INFO] [$(date +"%Y-%m-%d %H:%M:%S")] Building enable metrics Helm set ..." >> logzio_agent.log
...
function build_logzio_metrics_listener_url_helm_set () {
    echo -e "[INFO] [$(date +"%Y-%m-%d %H:%M:%S")] Building Logz.io metrics listener URL Helm set ..." >> logzio_agent.log
...
function build_logzio_metrics_token_helm_set () {
    echo -e "[INFO] [$(date +"%Y-%m-%d %H:%M:%S")] Building Logz.io metrics token Helm set ..." >> logzio_agent.log
...
function get_is_k8s_runs_on_windows_os () {
    echo -e "[INFO] [$(date +"%Y-%m-%d %H:%M:%S")] Getting if Kubernetes runs on Windows OS ..." >> logzio_agent.log
    ...
    echo -e "[INFO] [$(date +"%Y-%m-%d %H:%M:%S")] is_windows = $is_windows_value" >> logzio_agent.log
...
function build_windows_node_username_and_password_helm_sets () {
    echo -e "[INFO] [$(date +"%Y-%m-%d %H:%M:%S")] Building Windows Node username and password Helm sets ..." >> logzio_agent.log
    ...
    echo -e "echo -e \"[INFO] [$(date +"%Y-%m-%d %H:%M:%S")] windows_sets = \$windows_sets\" >> logzio_agent.log" >> logzio-temp/run

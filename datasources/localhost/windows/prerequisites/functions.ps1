...
function Test-CanLocalhostConnectToLogzioLogs {
    ...
    $local:LogzioListenerUrl = &$JqExe -r .listenerUrl $AppJson
    if ($LASTEXITCODE -ne 0) {
        $Message = "prerequisites.ps1 ($ExitCode): error getting listener url from application json: $_"
    ...
    try {
        ...
        Send-LogToLogzio $LogLevelError $Message $LogStepPreInit $LogScriptAgent $FuncName $AgentId $Platfrom $Subtype
